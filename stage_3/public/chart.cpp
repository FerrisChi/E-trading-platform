...
Chart::Chart(int index) {
  std::ifstream ifs("../data/chart.data",
...
void Chart::Write() const {
  std::fstream fs("../data/chart.data",
...
QDataStream &operator<<(QDataStream &out, Chart &chart) {
  out << chart.customerID << chart.size;
  for (int i = 0; i < MAX_CHART_SIZE; i++) out << chart.product[i];
  return out;
}

QDataStream &operator>>(QDataStream &in, Chart &chart) {
  in >> chart.customerID >> chart.size;
  for (int i = 0; i < MAX_CHART_SIZE; i++) in >> chart.product[i];
  return in;
}
